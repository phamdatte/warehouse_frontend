...
export default function ReportIssuePage() {
    ...
    const fetchPage = useCallback(async (p = 0) => {
        ...
        try {
            ...
            toast.error('Failed to load issue report');
        } finally {
            setLoading(false);
        }
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const rows = (res.data.content || []).map((r) => ({
                'Issue No.':          r.issueNumber || '',
                'Customer':           r.customerName || '',
                'Issue Date':         r.issueDate ? new Date(r.issueDate).toLocaleDateString('en-US') : '',
                'Status':             r.status || '',
            ...
            XLSX.writeFile(wb, `issue_report_${applied.month || new Date().toISOString().slice(0, 10)}.xlsx`);
    ...
    return (
        <div>
            <PageHeader title="Goods Issue Report" subtitle={`${totalElements} approved issues`}>
