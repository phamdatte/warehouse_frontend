    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
                <h3 className="text-lg font-semibold mb-4">{customer?.customerId ? 'Edit Customer' : 'Add Customer'}</h3>
                <form onSubmit={handleSubmit} className="space-y-3">
                    {[['customerName', 'Customer Name *', true], ['contactPerson', 'Contact Person', false], ['phone', 'Phone', false], ['email', 'Email', false], ['address', 'Address', false]].map(([name, label, req]) => (
                        <div key={name}>
...
export default function CustomerPage() {
    ...
    const handleDelete = async () => {
        try { await masterApi.deleteCustomer(deleteTarget.customerId); toast.success('Deleted successfully!'); setDeleteTarget(null); fetch(page); }
        catch (err) { toast.error(err.response?.data?.message || 'Failed to delete'); }
    };
    const handleToggle = async (row) => {
        try {
            await masterApi.toggleCustomer(row.customerId);
            toast.success(`Customer "${row.customerName}" ${row.isActive ? 'deactivated' : 'activated'}!`);
            fetch(page);
    ...
    return (
        <div>
            <PageHeader title="Customers" subtitle={`${totalElements} customers`}>
            ...
            <ConfirmModal isOpen={!!deleteTarget} title="Delete customer" message={`Are you sure you want to delete customer "${deleteTarget?.customerName}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
        </div>
    );
}
